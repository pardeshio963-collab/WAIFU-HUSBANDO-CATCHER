...
async def _resolve_coin_target(update: Update, context: CallbackContext):
    if update.message.reply_to_message:
...
def _coin_amount_arg(update: Update, context: CallbackContext):
    if update.message.reply_to_message:
        return context.args[0] if context.args else None
    return context.args[1] if len(context.args) >= 2 else None


async def addcoins(update: Update, context: CallbackContext) -> None:
    if update.effective_user.id not in Config.all_sudo():
        return

    target = await _resolve_coin_target(update, context)
    amount_arg = _coin_amount_arg(update, context)

    if not target or not amount_arg:
        await update.message.reply_text(
            "Usage:\n"
            "• Reply: <code>/addcoins 500</code>\n"
            "• Username: <code>/addcoins @username 500</code>",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    await _ensure_user(target.id, target)
    await user_collection.update_one(
        {"id": target.id},
        {"$inc": {"coins": amount}},
    )
    await update.message.reply_text(
        f"✅ Added <b>{amount:,}</b> coins to "
        f"<b>{escape(target.first_name or target.username or str(target.id))}</b>.",
        parse_mode=ParseMode.HTML,
    )


async def removecoins(update: Update, context: CallbackContext) -> None:
    if update.effective_user.id not in Config.all_sudo():
        return
    ...
    if not target or not amount_arg:
        await update.message.reply_text(
            "Usage:\n"
            "• Reply: <code>/removecoins 500</code>\n"
            "• Username: <code>/removecoins @username 500</code>",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    await update.message.reply_text(
        f"✅ Removed <b>{amount:,}</b> coins from "
        f"<b>{escape(target.first_name or target.username or str(target.id))}</b>.",
        parse_mode=ParseMode.HTML,
    )
...
application.add_handler(CommandHandler("addcoins", addcoins, block=False))
application.add_handler(CommandHandler("removecoins", removecoins, block=False))
