...
from waifu import application, user_collection, waifu_collection, sudo_users, OWNER_ID, GROUP_ID, UPDATE_CHAT, SUPPORT_CHAT
...
async def _check_required_joins(update: Update, context: CallbackContext) -> bool:
    """Require the user to join the official GC and update channel before opening harem."""
    ...
    try:
        ...
        checks.append(member.status not in ("left", "kicked"))
    except Exception:
        checks.append(False)

    # Official update channel
    update_chat = UPDATE_CHAT.strip().lstrip("@")
    try:
        member = await context.bot.get_chat_member(
            f"@{update_chat}" if update_chat else UPDATE_CHAT, user_id
        )
        checks.append(member.status not in ("left", "kicked"))
    except Exception:
        checks.append(False)
    ...
    rows = []
    if not checks[0]:
        rows.append([InlineKeyboardButton("💬 Join Official GC", url=f"https://t.me/{SUPPORT_CHAT}")])
    if not checks[1] and update_chat:
        rows.append([InlineKeyboardButton("📢 Join Update Channel", url=f"https://t.me/{update_chat}")])
    rows.append([InlineKeyboardButton("✅ Check Again", callback_data=f"hjoin:{user_id}")])

    text = (
        "🔒 <b>Harem Locked</b>\n\n"
        "Join the official GC and update channel first.\n"
        "Then tap <b>Check Again</b>."
    )
...
async def harem(update: Update, context: CallbackContext, page: int = 0) -> None:
    if not await _check_required_joins(update, context):
        return
    user_id = update.effective_user.id
...
async def harem_join_callback(update: Update, context: CallbackContext) -> None:
    q = update.callback_query
    try:
        uid = int(q.data.split(":", 1)[1])
    except Exception:
        await q.answer("❌ Invalid check.", show_alert=True)
        return
    if q.from_user.id != uid:
        await q.answer("❌ That's not your harem!", show_alert=True)
        return
    if await _check_required_joins(update, context):
        await q.answer("✅ Verified!")
        await harem(update, context, page=0)
    else:
        await q.answer("❌ Join both first.", show_alert=True)
