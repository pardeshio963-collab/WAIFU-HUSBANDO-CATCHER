...
WATERMARK_PATH = Path(__file__).with_name("waifu_nexus_logo.png")
WATERMARK_MAX_RATIO = 0.09
WATERMARK_MARGIN_RATIO = 0.018
MAX_IMAGE_SIDE = 1600

async def _watermark_photo(bot, file_id: str) -> io.BytesIO:
    """Download, lightly resize, watermark, and compress an image for fast Telegram upload."""
    ...
    # Keep large uploads fast while preserving good quality.
    if max(base.size) > MAX_IMAGE_SIDE:
        scale = MAX_IMAGE_SIDE / max(base.size)
        base = base.resize(
    ...
    # Small premium watermark: about 9% of image width.
    target_w = max(36, int(base.width * WATERMARK_MAX_RATIO))
    scale = target_w / logo.width
    target_h = max(1, int(logo.height * scale))
    logo = logo.resize((target_w, target_h), Image.Resampling.LANCZOS)

    margin = max(6, int(base.width * WATERMARK_MARGIN_RATIO))
...
async def _watermark_and_send_photo(bot, chat_id: int, file_id: str, **kwargs):
    """Send a watermarked copy with generous Telegram network timeouts."""
...
async def _send_original_photo(bot, chat_id: int, file_id: str, **kwargs):
    """Send the original Telegram image without applying any watermark."""
    return await bot.send_photo(
        chat_id=chat_id,
        photo=file_id,
        read_timeout=60,
...
# ── Method 1: /upload (original image, no watermark) ─────────────────────────
...
async def upload(update: Update, context: CallbackContext) -> None:
    ...
    try:
        msg = await _send_original_photo(
            context.bot,
            CHARA_CHANNEL_ID,
            photo,
...
# ── Method 2: /uploadw (same as /upload, with Waifu Nexus watermark) ─────────

async def uploadw(update: Update, context: CallbackContext) -> None:
    if not _is_sudo(update.effective_user.id):
        await update.message.reply_text("❌ Sudo only.")
        return
    ...
    if len(context.args) not in (3, 4):
        await update.message.reply_text(
            "❌ Wrong format\n\n"
            "<code>/uploadw name anime rarity [edition]</code>\n\n"
            "💡 Edition is optional.\n"
            "<i>Reply to the character image when using /uploadw.</i>",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    replied = update.message.reply_to_message
    if not replied:
        await update.message.reply_text("❌ Reply to the character image with /uploadw.")
        return
